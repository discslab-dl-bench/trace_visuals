...
def preprocess_data(data_dir, workload):
    ...
    step_analysis_dir = data_dir / "step_analysis"
    step_analysis_dir.mkdir(exist_ok=True, parents=True)

    sleep_times_dir = data_dir / "sleep_times"
    sleep_times_dir.mkdir(exist_ok=True, parents=True)

    outfile = open(os.path.join(step_analysis_dir, f"{workload}_step_analysis_{timestamp}.log"), "w")
    ...
    for log_file in log_files:
        ...
        for key in current_file_times:
            ...
            ROUND = 5
            outfile.write(f"{key:>30}:\t{round(mean, ROUND):>15}\t{round(std, ROUND):>15}\t{round(quartiles[0], ROUND):>15}\t{round(quartiles[1], ROUND):>15}\t{round(quartiles[2], ROUND):>15}\n")
        outfile.write("\n")
    ...
    with open(os.path.join(sleep_times_dir, f"{workload}_sleep_times_{timestamp}.json"), "w") as outfile:
...
if __name__ == '__main__':
    ...
    plot_data_loading(plotting_data, workload)
